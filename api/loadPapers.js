...
async function loadPapers() {
    ...
    fs.readdir(papersDirectory, async (err, files) => {                 // lists all files within /api/papers directory (all the html files)
        ...
        for (let file of files) {                                       // loop over each file in the directory of html papers
            ...
            const htmlContent = fs.readFileSync(filePath, 'utf8');      // read contents of html file, store in 'htmlcontent'
            const $ = cheerio.load(htmlContent);                        // loads html content into Cheerio for parsing 

            const title = $('h1.paper__title').text().trim(); // extract title of paper
            ...
            $('li.paper__meta-item').each((index, element) => {     // iterates over each <li> element with class 'paper__meta-item'
                ...
                // Check if the text is a number. if it is, save this as the year.
